...
int ptex_fun(float u, float v, GzColor color) // currently set to checkerboard
{
	...
	float turbsize, Xlines, Ylines, turbPOW;

	
	// soft_tex = 1
	//agressive_tex = 2
	//straight line marble texture=3
	...
	while (turbsize >= 1)
	{
		noise_value += perlinNoise(x*freq, y*freq) * turbsize;
		total += ampl;
		freq *= 2.0f;
		ampl *= persist;
		turbsize /= 2.0;
	...
	noise_value = fabs(noise_value);
		...
		float xCoord = u * GSIZE;
		float yCoord = v * GSIZE;
		
		float intermediate_tex = xCoord * Xlines / GSIZE + yCoord * Ylines / GSIZE + turbPOW*noise_value / 256;

		float new_texture = 0.701*fabs(sin(intermediate_tex * 3.14159265358979));

		float diffR = 0.6;
		float diffG = 0.8;
		float diffB = 0.4;
		color[RED] = (0.3 + diffR*new_texture);
		color[GREEN] = (0.3 + diffG*sqrt(new_texture));
		color[BLUE] = (0.6 + diffB*sqrt(new_texture));
